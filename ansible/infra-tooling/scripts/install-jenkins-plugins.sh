...
download() {
    ...
    if [[ $ignoreLockFile ]] || sudo mkdir "$lock" &>/dev/null; then
...
doDownload() {
    ...
    if [[ "$version" == "latest" && -n "$JENKINS_LATEST" ]]; then
        ...
        if [[ "$version" == "latest" ]]; then
           url="$JENKINS_LATEST/plugins/${plugin}.hpi"
        else
          JENKINS_UC_DOWNLOAD=${JENKINS_UC_DOWNLOAD:-"$JENKINS_UC/releases/org/jenkins-ci"}
          if [[ $plugin == "workflow"*  ]] ; then
             url="$JENKINS_UC_DOWNLOAD/plugins/workflow/$plugin/$version/${plugin}-${version}.hpi"
          else
             url="$JENKINS_UC_DOWNLOAD/plugins/$plugin/$version/${plugin}-${version}.hpi"
          fi
        fi
    fi

    echo "Downloading plugin: $plugin from $url"
    retry_command sudo curl "${CURL_OPTIONS:--sSfL}" --connect-timeout "${CURL_CONNECTION_TIMEOUT:-20}" --retry "${CURL_RETRY:-5}" --retry-delay "${CURL_RETRY_DELAY:-0}" --retry-max-time "${CURL_RETRY_MAX_TIME:-60}" "$url" -o "$jpi"
    return $?
}
...
main() {
    ...
    if curl -fsL -o /dev/null "$JENKINS_LATEST/$jenkinsVersion"; then
        JENKINS_UC_LATEST="$JENKINS_LATEST/$jenkinsVersion"
